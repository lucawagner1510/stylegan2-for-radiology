...
def upsample_to_size(x):
    ...
    y = int(im_size / x.shape[2])  # it was a bug without cast to int !!
...
def g_block(inp, istyle, inoise, fil, u=True):
    if u:
        ...
        out = Lambda(
            upsample, output_shape=[None, inp.shape[2] * 2, inp.shape[2] * 2, None]
        )(inp)
    else:
        out = Activation("linear")(inp)

    rgb_style = Dense(fil, kernel_initializer=VarianceScaling(200 / out.shape[2]))(
        istyle
    )
    ...
    out = Conv2DMod(
        filters=fil, kernel_size=3, padding="same", kernel_initializer="he_uniform"
    )([out, style])
    out = add([out, d])
...
class StyleGAN(object):
    ...
    def train(self):
        ...
        a, b, c, d = self.train_step(
            self.im.get_batch(BATCH_SIZE).astype("float32"),
            style,
            nImage(BATCH_SIZE),
        ...
        if self.GAN.steps % 100 == 0 and not self.silent:
            ...
            if self.GAN.steps % 1000 == 0 or (
                self.GAN.steps % 100 == 0 and self.GAN.steps < 2500
            ):
        ...
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            ...
            if perform_pl:
                ...
                for i in range(len(style)):
                    ...
                    w_space_2.append(
                        w_space[i]
                        + K.random_normal(tf.shape(w_space[i])) / (std + 1e-8)
                    )
    ...
    def evaluate(self, num=0, trunc=1.0):
        ...
        x.save("/content/drive/My Drive/stylegan/Results/i" + str(num) + ".png")
        ...
        generated_images = self.GAN.GMA.predict(
            latent + [nImage(4), trunc], batch_size=BATCH_SIZE
        )
        ...
        x.save("/content/drive/My Drive/stylegan/Results/i" + str(num) + "-mr.png")

    def generateTruncated(
        self, style, noi=np.zeros([44]), trunc=0.5, outImage=False, num=0
    ):
        ...
        if outImage:
            ...
            x.save("/content/drive/My Drive/stylegan/Results/t" + str(num) + ".png")
    ...
    def saveModel(self, model, name, num):
        json = model.to_json()
        with open(
            "/content/drive/My Drive/stylegan/Models/" + name + ".json", "w"
        ) as json_file:
            json_file.write(json)

        model.save_weights(
            "/content/drive/My Drive/stylegan/Models/" + name + "_" + str(num) + ".h5"
        )

    def loadModel(self, name, num):

        file = open("/content/drive/My Drive/stylegan/Models/" + name + ".json", "r")
        json = file.read()
        file.close()
