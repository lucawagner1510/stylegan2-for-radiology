...
from tensorflow.keras.layers import (Lambda, Flatten, Activation,Conv2D, Dense, add, LeakyReLU,
                                     AveragePooling2D, concatenate, Reshape)
...
class StyleGAN(object):
    ...
    def saveModel(self, model, name, num):
        json = model.to_json()
        with open("/content/drive/My Drive/stylegan/Models/"+name+".json", "w") as json_file:
            json_file.write(json)

        model.save_weights("/content/drive/My Drive/stylegan/Models/"+name+"_"+str(num)+".h5")
    ...
    def loadModel(self, name, num):
        ...
        mod.load_weights("/content/drive/My Drive/stylegan/Models/"+name+"_"+str(num)+".h5")
...
if __name__ == "__main__":
    ...
    model = StyleGAN(lr = 0.00001, silent = False)
    #model.load(5)
    model.evaluate(0)
